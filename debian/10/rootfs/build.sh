...
SECURITY_MIRROR="${SECURITY_MIRROR:-http://archive.debian.org/debian-security}"
IMAGE_TAG="${IMAGE_TAG:-localhost/debian:buster-${SUFFIX}}"
...
if [ -n "${QEMU_BIN}" ]; then
  if command -v update-binfmts >/dev/null 2>&1; then
    sudo update-binfmts --enable "${QEMU_BIN%-static}" || true
  fi
...
deb ${SECURITY_MIRROR} buster/updates main contrib non-free
deb ${MIRROR} buster-updates main contrib non-free
EOF

sudo mkdir -p "${ROOTFS}/etc/apt/apt.conf.d"
sudo tee "${ROOTFS}/etc/apt/apt.conf.d/99debian-archive" >/dev/null <<'EOF'
